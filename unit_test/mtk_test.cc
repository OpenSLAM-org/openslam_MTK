...
BOOST_AUTO_TEST_CASE( SO3 )
{
	...
	for(int i=0; i<16; ++i)
	{
		...
		SO3 z = y.inverse();
		SO3 minus_one = SO3::base(-one, 0, 0, 0);
		BOOST_CHECK_CLOSE(x.norm(), one, tol);
		BOOST_CHECK_CLOSE(y.norm(), one, tol);
		BOOST_CHECK_CLOSE(z.norm(), one, tol);
...
BOOST_AUTO_TEST_CASE( S2 )
{
	...
	for(int i=0; i<16; ++i)
	{
		...
		S2 z(-2.0, 0,0);
		BOOST_CHECK_CLOSE(x.get_vect().norm(), scalar(1.0), tol);
		BOOST_CHECK_CLOSE(y.get_vect().norm(), scalar(1.0), tol);
		BOOST_CHECK_CLOSE(z.get_vect().norm(), scalar(1.0), tol);
